...
def mtl_f1(predicted, actual, type = 'micro'):
    ...
    # predicted = tf.cast(predicted, tf.int8)
    # actual = tf.cast(actual, tf.int8)
    
    TP = tf.count_nonzero(predicted * actual, 0)
    # TN = tf.count_nonzero((predicted - 1) * (actual - 1), 0)
    FP = tf.count_nonzero(predicted * (1 - actual), 0)
    FN = tf.count_nonzero((1 - predicted) * actual, 0)
...
class GNNMetaApprox(GNNAttack):
    ...
    """
    def __init__(self, adjacency_matrix, attribute_matrix, labels_onehot, hidden_sizes, train_iters=100, gpu_id=None,
                 _lambda=0.5, dtype=tf.float32, isMTL = False):
        """
    ...
    def build(self, with_relu=False, learning_rate=1e-2):
        ...
        with self.graph.as_default():
            ...
            if not self.isMTL:
                ...
                self.classification_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=labels_gather,
                                                                                                 logits=logits_gather))
            epsilon = 1e-8
...
class GNNMeta(GNNAttack):
    ...
    def __init__(self, adjacency_matrix, attribute_matrix, labels_onehot, hidden_sizes, train_iters=100, gpu_id=None,
                 attack_features=False, dtype=tf.float32, isMTL = False):
        """
    ...
    def make_loss(self, ll_constraint=True, ll_cutoff=0.004):
        ...
        with self.graph.as_default():
            ...
            if not self.isMTL:
                ...
                attack_loss_per_node = tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_attack,
                                                                                labels=labels_atk)
...
class GCNSparse:
    ...
    def __init__(self, adjacency_matrix, attribute_matrix, labels_onehot, hidden_sizes, isMTL = False, gpu_id=None):
        """
        Parameters
        ----------
        ...
        with self.graph.as_default():
            ...
            for ix in range(len(self.hidden_sizes)):
                ...
                if ix == 0 and self.sparse_attributes:
                    ...
                    hidden = tf.sparse_tensor_dense_matmul(self.adj_norm, hidden @ w) + b
    ...
    def change_A(self, new_A, with_relu=True):
        # assert type(new_A) == type(self.adj_norm), 'Wrong type: new_A must be a SparseTensor'
        with self.graph.as_default():
            self.adj_norm = tf.SparseTensor(new_A[0], new_A[1], new_A[2])
            hidden = self.attributes
            for ix in range(len(self.hidden_sizes)):
                w = self.weights[ix]
                b = self.biases[ix]
    ...
    def mean(self, a_list_of_gcns, with_relu = True):
        # update weight
        for ix in range(len(self.weights)):
            weight_i = []
            bias_i = []
            for gcn in a_list_of_gcns:
                session = gcn.session
                weight_i.append(gcn.weights[ix].eval(session=session))
                bias_i.append(gcn.biases[ix].eval(session=session))
            with self.graph.as_default():
                self.weights[ix] = tf.Variable(np.mean(weight_i, 0))
                self.biases[ix] = tf.Variable(np.mean(bias_i, 0))
        # update logits
    ...
    def train_with_validation(self, idx_train, idx_val, n_iters=200, early_stop = True, initialize=True, display=True):
        with self.graph.as_default():
            if initialize:
                self.session.run(tf.global_variables_initializer())
            ...
            for _it in _iter:
                self.session.run(self.train_op, feed_dict={self.idx: idx_train})
                val_acc = (self.logits.eval(session=self.session).argmax(1) == np.argmax(self.labels_onehot, axis=-1))[idx_val].mean()
                if early_stop:
...
class GATSparse:
    """
    GAT implementation with a sparse adjacency matrix and possibly sparse attribute matrices.
    """
        ...
        with self.graph.as_default():
            ...
            for ix in range(len(self.hidden_sizes)):
                ...
                for hd in range(head[ix]):
                    ...
                    attn_scores[ix].append((origin_attn_avg - ptb_attn_avg, origin_attn_avg, ptb_attn_avg))
