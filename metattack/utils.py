...
def all_connected_components(adj):
    """Select all connected components in the graph, sorted by # of nodes
    Parameters
    ----------
    ...
    components_to_keep = np.argsort(component_sizes)[::-1]  # reverse order to sort descending
    ccs = [[] for _ in range(components_to_keep.shape[0])]
    for (idx, component) in enumerate(component_indices):
        ccs[component].append(idx)
    return ccs, components_to_keep
